function [ideal_angle,NOM_angle,AVG_angle,LR_angle] = err_noise_run(param,train)
%% parameters %%
%runtime parameters
NT = 10^5; % number of arrays used for training
NB = 3; % number of angles
N = 10^3; % phased array estimator

%Array parameters
NE = 30; % number of elements in the array
lambda = 1; % wavelength
D = 1; % distance in y axes between elements
boundery = 2; % in degrees
meas_noise = param; % stdv of mesurment device noise
dev_sigma = 0.2; % stdv of deviations for training
dev_max = 0.3; % maximal deviation
curve_sigma = 0.2; % stdv of curve for training
curve_max = 0.5; % maximal curve


%% Untested input %%
%parameters for untesteded array
dev_sigma_test = 0.1; % stdv of deviations for test
curve_test = 0.7*truncnormrnd(1, 0, curve_sigma, -curve_max, curve_max); % curve for test
